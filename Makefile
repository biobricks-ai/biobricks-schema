...
1 :=
unquote = $(subst $(ESC),$(SPACE),$(patsubst "%,%,$(patsubst %",%,$(subst $(SPACE),$(ESC),$(1)))))
...
SHEET_TABS = $(call unquote,$(LINKML_SCHEMA_GOOGLE_SHEET_TABS))
...
examples/output: src/biobricks_schema/schema/biobricks_schema.yaml
	mkdir -p $@
